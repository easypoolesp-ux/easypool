...
def seed_isolation():
    ...
    school1, _ = School.objects.get_or_create(
        name='Oakridge International', defaults={'address': 'Gachibowli, Hyderabad'}
    )

    # Transporter 1 for School 1
    t1_s1, _ = Transporter.objects.get_or_create(
        name='Oakridge Fleet A', school=school1, defaults={'email': 'fleeta@oakridge.com'}
    )

    # Transporter 2 for School 1
    t2_s1, _ = Transporter.objects.get_or_create(
        name='Oakridge Fleet B', school=school1, defaults={'email': 'fleetb@oakridge.com'}
    )

    # Users for School 1
    u1, _ = User.objects.get_or_create(
        email='admin@oakridge.edu',
        defaults={'full_name': 'Oakridge Admin', 'role': 'school_admin', 'school': school1},
    )
    ...
    school2, _ = School.objects.get_or_create(
        name='Glendale Academy', defaults={'address': 'Sun City, Hyderabad'}
    )

    # Transporter 1 for School 2
    t1_s2, _ = Transporter.objects.get_or_create(
        name='Glendale Logistics', school=school2, defaults={'email': 'logistics@glendale.com'}
    )

    # Users for School 2
    u3, _ = User.objects.get_or_create(
        email='admin@glendale.edu',
        defaults={'full_name': 'Glendale Admin', 'role': 'school_admin', 'school': school2},
    )
