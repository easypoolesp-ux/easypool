...
async def init_clients():
    ...
    http_session = aiohttp.ClientSession(timeout=timeout, headers={"X-API-KEY": API_KEY})
...
async def forward_to_backend(imei: str, data: dict):
    ...
    # 1. Redis Publish (Non-blocking)
    if redis_client:
        try:
            await redis_client.publish('live_bus_updates', json.dumps(payload))
        except: pass

    # 2. HTTP Forward (Non-blocking)
    if http_session:
        try:
            async with http_session.post(BACKEND_API_URL, json=payload) as resp:
                if resp.status not in [200, 201]:
                    print(f"[ERROR] Backend returned {resp.status}")
        except: pass
