...
export default function BusDetailPage({ params }: Props) {
  ...
  if (!bus)
    return <div className="p-6 text-center text-red-500">Bus not found</div>;

  const getStatusInfo = (lastHeartbeat: string | null) => {
    if (!lastHeartbeat)
      return { label: "NO DATA", color: "text-slate-300", dot: "bg-slate-200" };
    ...
    return {
      label: `${Math.round(diffHours / 24)}d AGO`,
  ...
  const hlsUrl =
    selectedRecording && MEDIA_GATEWAY_URL
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {(() => {
            ...
            return (
              <div
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-border shadow-sm text-xs font-bold uppercase tracking-wider ${status.color}`}
              >
          ...
          <Card className="overflow-hidden border-none shadow-premium">
            ...
            <CardContent className="p-0 bg-black aspect-video relative">
              {!bus.cameras || bus.cameras.length === 0 ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900 z-10 text-center p-6 space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-slate-400 text-sm max-w-[280px]">
                      This vehicle does not have any active camera streams
                      assigned.
                    </p>
                  </div>
                </div>
                ...
                <LiveCamera
                  streamUrl={liveUrl}
                  title={activeCamera?.name || "Bus Camera"}
                />
              ) : (
                <>
                  <PlaybackCamera
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-10">
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-white font-medium">
                          Select a clip from the list to start playback
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <CardContent className="pb-6">
                ...
                <div className="mb-8 px-2">
                    ...
                    <div className="absolute inset-0 flex items-center px-1">
                      {recordings.map((rec) => {
                        ...
                        const percentage = Math.max(
                          0,
                          Math.min(100, (totalMinutes / (12 * 60)) * 100),
                        );
                          ...
                          <div
                            ...
                            className={`absolute h-6 w-1 rounded-full cursor-pointer transition-all hover:scale-y-125 ${
                              selectedRecording === rec.id
                                ? "bg-primary shadow-[0_0_10px_rgba(37,99,235,0.5)] z-20 h-8"
                                : "bg-primary/30 z-10"
                            }`}
                  ...
                  <p className="text-[10px] text-slate-500 mt-2 italic">
                    Tip: Click the vertical blue lines to jump to a specific
                    time.
                  </p>
                </div>
          ...
          <Card className="shadow-premium border-none">
            ...
            <CardContent className="space-y-4">
              <div className="aspect-square bg-muted rounded-xl overflow-hidden border border-border shadow-inner relative group">
                <FleetMap buses={[busMapData]} initialBusId={(bus as any).id} />
              </div>
              <div className="space-y-3 pt-2">
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground uppercase font-black text-[9px] tracking-tight">
                    Vehicle ID
                  </span>
