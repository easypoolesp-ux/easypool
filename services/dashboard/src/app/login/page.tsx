...
function clearAllSessionData() {
  ...
  document.cookie =
    "__session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC; SameSite=Lax; Secure";
}
  ...
  try {
    ...
    if (res.status === 403) {
      ...
      return {
        ...
        message:
          detail ||
          "Your account is not registered in EasyPool. Please contact your administrator.",
      };
    }
    return {
      ok: false,
    ...
    if (err?.name === "TimeoutError")
      return {
        ok: false,
        message: "Backend is unreachable. Please check your network.",
      };
    return {
      ok: false,
      message: "Authorization check failed. Please try again.",
...
export default function LoginPage() {
  ...
  async function handlePostFirebaseAuth(
    uid: string,
    method: "email" | "google",
  ) {
    ...
    console.log(
      "[LOGIN] Cookie and localStorage set, navigating to dashboard...",
    );
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[420px]">
          ...
          <div className="flex-1 p-8 flex flex-col justify-center items-center space-y-5">
            ...
            <p className="text-center text-xs text-slate-600 px-4">
              Access is invite-only. Contact your EasyPool administrator to
              request access.
            </p>
          </div>
        </div>
